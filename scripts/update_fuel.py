...
import time
import re
import random # НОВО: За случайни паузи
...
# URL адреси
# Махнахме счупените "Backup" линкове, които дават 404.
# За MK и BG оставяме backup, защото там работят.
URLS = {
    "BG": [ "https://bg.fuelo.net/?lang=en" ],
    "GR": [ "https://gr.fuelo.net/?lang=en" ],
    "RO": [ "https://ro.fuelo.net/?lang=en" ], # Махнат е счупения backup
    "TR": [ "https://tr.fuelo.net/?lang=en" ], # Махнат е счупения backup
...
def scrape_url(url, country_code, rates):
    scraper = cloudscraper.create_scraper()
    try:
        print(f"  Trying: {url} ...")
...
def main():
    ...
    for country in data.get('countries', []):
        ...
        if c_id in URLS:
            ...
            for index, url in enumerate(URLS[c_id]):
                # Ако е втори опит (backup), чакаме малко преди него
                if index > 0:
                    time.sleep(2) 
                
                new_prices = scrape_url(url, c_id, rates)
                if new_prices:
                    new_prices['last_updated'] = datetime.now().strftime("%Y-%m-%d")
                    country['fuel_prices'] = new_prices
                    print(f"  > SUCCESS: Found {len(new_prices)-1} fuel types.")
                    updated_count += 1
                    success = True
                    break
                else:
                    print("  > No data found, trying next URL (if available)...")
            
            if not success:
                print(f"  WARNING: Failed to fetch prices for {c_id}")
            
            # ВАЖНО: Случайна пауза между 3 и 6 секунди между държавите!
            # Това "заблуждава" защитата, че си човек.
